(function () {
  ...
  class ElementInspector {
    ...
    static getClassSelector(element) {
      // Strip Angular form-state classes — they change on every interaction
      // and make selectors fail on fresh page loads
      const classes = [...element.classList].filter(
        (cls) => Boolean(cls) && !/^ng-(dirty|pristine|valid|invalid|touched|untouched|pending|submitted)$/.test(cls),
      );
